...
export default function CompanyDetailBoard() {
    ...
    const applyCompanyToState = (found: Company) => {
        setCompany(found);
        ...
        setDiscipline(disciplineToDisplay(found.discipline));
        setTargetSponsorshipTier(priorityToDisplay(found.targetSponsorshipTier));
    ...
    const fetchData = async (forceRefresh = false) => {
        ...
        try {
            const data = await fetchSheetData(forceRefresh, controller.signal);
            clearTimeout(timeoutId);

            const companies: Company[] = (data.companies as Company[]) || [];
    ...
    useEffect(() => {
        if (companyId) {
            ...
            if (cached) {
                const companies = (cached.companies as Company[]) || [];
                const found =
                    fromSource === 'email-schedule'
                        ? companies.find(c => c.id === companyId)
                        : companies.find(c => c.id === companyId || c.companyName === companyId);
                if (found) applyCompanyToState(found);
            }
            fetchData();
        }
    ...
    if (loading && !company) {
        return <PageContentLoader label="Loading company…" />;
    }
