...
import { AnalyticsSkeleton } from './PageSkeletons';
import {
    ChartBarIcon,
    ArrowTrendingUpIcon,
    UserGroupIcon,
...
function readInitialAnalyticsState(): {
    ...
    if (cached && Array.isArray(cached.payload.companies) && cached.payload.companies.length > 0) {
        return {
            data: cached.payload.companies as Company[],
            dailyStats: cached.payload.dailyStats || [],
            committeeMembers: (cached.payload.committeeMembers || []) as CommitteeMember[],
            loading: false,
        };
    }
    return { data: [], dailyStats: [], committeeMembers: [], loading: true };
}
...
export default function AnalyticsBoard() {
    ...
    const initialAnalytics = useMemo(() => readInitialAnalyticsState(), []);
    const [data, setData] = useState<Company[]>(initialAnalytics.data);
    const [history, setHistory] = useState<HistoryEntry[]>([]);
    const [dailyStats, setDailyStats] = useState<any[]>(initialAnalytics.dailyStats);
    const [committeeMembers, setCommitteeMembers] = useState<CommitteeMember[]>(initialAnalytics.committeeMembers);
    ...
    const fetchData = async (refresh = false) => {
        ...
        try {
            ...
            setHistory((responseData.history as HistoryEntry[]) || []);
            setDailyStats(responseData.dailyStats || []);
            setCommitteeMembers((responseData.committeeMembers as CommitteeMember[]) || []);
        } catch (err) {
    ...
    useEffect(() => {
        // Prefer in-memory/shared cache so the spinner often clears quickly,
        // but still wait for fetchSheetData before revealing the board.
        if (initialAnalytics.data.length === 0) {
            ...
            if (inMemory && Array.isArray(inMemory.companies) && inMemory.companies.length > 0) {
                ...
                setCommitteeMembers((inMemory.committeeMembers || []) as CommitteeMember[]);
            }
        }
        fetchData();
    }, []);
