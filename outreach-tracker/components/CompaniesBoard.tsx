...
export default function CompaniesBoard() {
    ...
    const { fetchSheetData, invalidate: invalidateSheetData } = useSheetData();
    ...
    const [trackerOnlyCompanies, setTrackerOnlyCompanies] = useState<Array<{ id: string; name: string }>>([]);
    ...
    const fetchData = async (
        ...
        try {
            const responseData = await fetchSheetData(forceRefresh, controller.signal);
            clearTimeout(timeoutId);

            setData((responseData.companies as Company[]) || []);
            setIdNameMismatches(forceRefresh && responseData.idNameMismatches ? (responseData.idNameMismatches as { id: string; trackerName: string; dbName: string }[]) : []);
            setTrackerOnlyCompanies((responseData.trackerOnlyCompanies as { id: string; name: string }[]) || []);
            persistCompaniesPayload(responseData as { companies?: Company[]; idNameMismatches?: { id: string; trackerName: string; dbName: string }[]; trackerOnlyCompanies?: { id: string; name: string }[] });
    ...
    const confirmBulkAssign = async () => {
        ...
        try {
            ...
            setIsSyncing(true);
            try {
                const [dataResult, scheduleRes] = await Promise.all([
                    fetchSheetData(true),
                    fetch('/api/email-schedule'),
                ]);
                setData((dataResult.companies as Company[]) || []);
                persistCompaniesPayload(dataResult as { companies?: Company[] });
                if (scheduleRes.ok) {
    ...
    const confirmBulkStatusUpdate = async () => {
        ...
        try {
            ...
            setIsSyncing(true);
            try {
                const json = await fetchSheetData(true);
                setData((json.companies as Company[]) || []);
                persistCompaniesPayload(json as { companies?: Company[] });
                updateTaskProgress(taskId, 100, { current: count, total: count });
            } finally {
                setIsSyncing(false);
            }
    ...
    const handleApplyBatchLabel = async () => {
        ...
        try {
            ...
            // This is an optimistic local update only — the shared cache still
            // holds the pre-label snapshot, so mark it stale or other pages
            // opened within the cache TTL would silently show the old label.
            invalidateSheetData();
    ...
    if (loading && data.length === 0) {
        return <PageContentLoader label="Loading company database…" />;
    }
    ...
    return (
        <>
                    ...
                    <div className="flex items-start gap-3">
                        ...
                        <div className="flex-1 min-w-0">
                            <p className="text-sm font-medium text-amber-800">
                                {trackerOnlyCompanies.length} company(ies) exist in Tracker but not in the Database
                            </p>
                            <p className="text-xs text-amber-700 mt-1">
                                Sample: {trackerOnlyCompanies.slice(0, 5).map(c => `${c.id} (${c.name || '—'})`).join(', ')}
                                {trackerOnlyCompanies.length > 5 && ` (+${trackerOnlyCompanies.length - 5} more)`}
                            </p>
                            <p className="text-xs text-amber-700 mt-1">
                                Run Sync Database in Settings to align these companies before they appear in the main list.
                            </p>
                            <a href="/settings" className="inline-block mt-2 text-sm font-medium text-amber-800 underline hover:text-amber-900">
                                Go to Settings
                            </a>
                        </div>
                        <button onClick={() => setTrackerOnlyCompanies([])} className="text-amber-600 hover:text-amber-800" aria-label="Dismiss">
            ...
            {/* ID/Name mismatch warning (checked on refresh; Database is source of truth) */}
            {idNameMismatches.length > 0 && (
                <div className="mb-4 rounded-lg bg-amber-50 border border-amber-200 px-4 py-3">
                    <div className="flex items-start gap-3">
                        <svg className="w-5 h-5 text-amber-600 flex-shrink-0 mt-0.5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-3L13.732 4c-.77-1.333-2.694-1.333-3.464 0L3.34 16c-.77 1.333.192 3 1.732 3z" />
                        ...
                        <div className="flex-1 min-w-0">
                            <p className="text-sm font-medium text-amber-800">
                                {idNameMismatches.length} company name mismatch(es) detected vs. Database (source of truth)
                            </p>
                            <p className="text-xs text-amber-700 mt-1">
                                Tracker names differ from the main list for: {idNameMismatches.slice(0, 5).map(m => `${m.id}`).join(', ')}
                                {idNameMismatches.length > 5 && ` (+${idNameMismatches.length - 5} more)`}
                            </p>
                            <a href="/settings" className="inline-block mt-2 text-sm font-medium text-amber-800 underline hover:text-amber-900">
                                Go to Settings
                            </a>
                        </div>
                        <button onClick={() => setIdNameMismatches([])} className="text-amber-600 hover:text-amber-800" aria-label="Dismiss">
                            <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M6 18L18 6M6 6l12 12" /></svg>
                        </button>
                    </div>
                </div>
